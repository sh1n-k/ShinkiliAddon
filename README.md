...
- Settings are stored **per character and per spec** (`Name-Realm` → spec): spell colour mappings, defense, procs, vitals thresholds, the exclusion lists and the SimC toggle all live there; the main/defense/vitals box placement and the exclusion keybind are per character. Only cast-state **overrides**, the interrupt-signal toggle, language and the minimap button are account-wide
